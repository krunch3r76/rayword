...
async def read_lines_from_stream(stream, pad, proc):
    ...
    while True:
        ...
        try:
            ...
            # line = await asyncio.wait_for(stream.readline(), timeout=0.05)
        ...
        while not exit_event.is_set():
            ...
            if ch == curses.KEY_DOWN:
                if current_line_offset > 0:
                    ...
                    if current_line_offset == 0:
                        if pad._padline == padline_at_keyup:
                            auto_scroll_active = True
                        else:
                            current_line_offset = pad._padline - padline_at_keyup
                    else:
                        update_event.set()
            elif ch == curses.KEY_UP:
                logging.debug(f"KEY_UP: current line offset is {current_line_offset}")
                if current_line_offset == 0:
                    padline_at_keyup = pad._padline
